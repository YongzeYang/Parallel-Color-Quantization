...
baseline_data <- data[data$Methodology == 'pure_spark',]
...
ggplot(baseline_data, aes(x=Different_Colors, y=Time_Cost)) +
  ...
  labs(title="Time Cost Using PURE_SPARK Methodology with Number of Colors Variation", x="Number of Colors", y="Time Cost(s)") +
  theme_minimal() +
